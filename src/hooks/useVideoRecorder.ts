...
export const useVideoRecorder = ({ audioElementRef, audioContextRef, sourceNodeRef }: UseVideoRecorderProps = {}) => {
  ...
  const startRecording = useCallback(async () => {
    try {
      ...
      if (audioElement && audioContextRef?.current && sourceNodeRef?.current) {
        // Resume AudioContext if suspended (required for audio capture)
        if (audioContextRef.current.state === 'suspended') {
          await audioContextRef.current.resume();
        }

        // Use existing AudioContext and source node from useAudioSystem
  ...
  const autoGenerateVideo = useCallback(async (_duration: number) => {
    try {
      ...
      // Small delay to ensure audio is flowing through the graph
      await new Promise(resolve => setTimeout(resolve, 100));

      // Start recording after audio is playing
      mediaRecorderRef.current = mediaRecorder;
